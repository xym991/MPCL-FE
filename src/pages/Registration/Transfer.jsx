...
export default function PlayerTransfer({ tab, setTab }) {
  const {
    register,
    handleSubmit,
    setValue,
  ...
  const onSubmit = async (data) => {
    try {
      const response = await axios.post(paths.player_transfer, {
        ...data,
        from_club: data.from_club.value,
        to_club: data.to_club.value,
      });
  ...
  return (
    <div className="_player-transfer container w-full py-8 px-16 flex flex-col justify-start">
      {/* <h1 className="heading text-center">Player Transfer</h1> */}

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-2 mt-4">
        <div className="space-y-2">
          <label>
            From Club <span className="text-red-500">*</span>
          </label>
          <Controller
            name="from_club"
            control={control}
            rules={{ required: "From Club is required" }}
            render={({ field }) => (
              <Select
                {...field}
                options={clubs}
          ...
          />
          {errors.from_club && (
            <p className="text-red-500">{errors.from_club.message}</p>
          )}
        </div>

        <div className="space-y-2">
          <label>
            To Club <span className="text-red-500">*</span>
          </label>
          <Controller
            name="to_club"
            control={control}
            rules={{ required: "To Club is required" }}
            render={({ field }) => (
              <Select
                {...field}
                options={clubs}
          ...
          />
          {errors.to_club && (
            <p className="text-red-500">{errors.to_club.message}</p>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          {errors.player_classification && (
            <p className="text-red-500">Player Classification is required</p>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          <p className="ml-4 md:ml-8 from-gray-500 text-sm">
            I confirm that I have read the regulation on player eligibility and
            qualification criteria and the disciplinary code of conduct of the
            MIDDLESEX PREMIER CRICKET LEAGUE.
            <br />I confirm that the information supplied by me may be held
            electronically for registration purpose and may be made available to
            other cricketing bodies.
          </p>
        </div>
        ...
        <div className="space-y-2">
          ...
          {errors.privacy && (
            <p className="text-red-500">You must agree to the privacy policy</p>
          )}
          <p className="ml-4 md:ml-8 from-gray-500 text-sm">
            I confirm that I have read the{" "}
            <Link to="/privacy-policy">privacy policy</Link> and consent to the
            given information being used by MPCL.
          </p>
        </div>
